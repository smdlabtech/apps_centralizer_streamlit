...
class LCLDetails:
    ...
    def display_summary(self):
        ...
        lcl_link_icon = app_styles.load_img_clickable("logo-lcl-rmvbg.png", link="https://monespace.lcl.fr/connexion", alt_text="LCL.fr", width=200)
        year_months = self.data['anneeOp'].unique().tolist()
        selected_month = st.selectbox("LCL : Mois ", year_months)
        st.write("You selected:", selected_month)
        
        # Filtrer les données en fonction du mois sélectionné
        filtered_data = self.data[self.data['anneeOp'] == selected_month]

        with st.expander("**📈 Financial KPIs [LCL]**"):
            self.display_financial_kpis(filtered_data)
        
        with st.expander("**Distribution of transactions [LCL]**"):
            self.display_transaction_type_pie_chart(filtered_data)

        with st.expander("**Transactions over time [LCL]**"):
            self.display_transactions_over_time(filtered_data)

        with st.expander("**Transactions by Day of the Week [LCL]**"):
            self.display_transactions_by_day(filtered_data)

        with st.expander("**Distribution of Creditors/Debtors [LCL]**"):
            self.display_creditor_debtor(filtered_data)
            
        with st.expander("**🔍 Metrics by Operation [LCL]**"):
            self.display_lcl_operation_text_metrics(filtered_data)


    ### CALCULS DE FONCTIONS
    def display_transaction_type_pie_chart(self, filtered_data):
        # st.write("**LCL** : Distribution of transactions by type")
        lcl_pie_chart = px.pie(filtered_data, names='typeOp', values='montant', title='Distribution of transactions by type')
        st.plotly_chart(lcl_pie_chart)

    def display_transactions_over_time(self, filtered_data):
        # st.write("**LCL** : Transactions over time")
        filtered_data['dateOp'] = pd.to_datetime(filtered_data['dateOp'])
        lcl_line_chart = px.line(filtered_data, x='dateOp', y='montant', title='Transactions over time', markers=True)
        st.plotly_chart(lcl_line_chart)

    def display_transactions_by_day(self, filtered_data):
        # st.write("**LCL** : Transactions par Jour de la Semaine")
        filtered_data['dayOfWeek'] = filtered_data['dateOp'].dt.day_name()
        lcl_day_expenses = filtered_data.groupby('dayOfWeek')['montant'].sum().reindex(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])
    ...
    def display_creditor_debtor(self, filtered_data):
        # st.write("**LCL** : Distribution of Creditors/Debtors")
        lcl_creditor_debtor = filtered_data['creancier'].value_counts()
    ...
    def lcl_metrique_org(self, filtered_data):
        grouped_df = filtered_data.groupby('operation_texte').agg(
            nombre=('montant', 'size'),
    ...
    def display_lcl_operation_text_metrics(self, filtered_data):
        ...
        if search_query:
            filtered_metric_data = self.lcl_metrique_org(filtered_data)[self.lcl_metrique_org(filtered_data).apply(lambda row: row.astype(str).str.contains(search_query, case=False).any(), axis=1)]
            occurrence = self.lcl_metrique_org(filtered_data).astype(str).apply(lambda x: x.str.contains(search_query, case=False)).sum().sum()
        ...
        ## Mise en forme de la sortie (LCL) ###
            ...
            kpi_data_lcl = self.lcl_metrique_org(filtered_data)
    ...
    #-------------------------------#
    def display_financial_kpis(self, filtered_data):
        ...
        for i, key in enumerate(kpi_keys):
            ...
            # kpi_value = round(self.kpis[value], 2)
            # if kpi_value < 0:
            #     value_color = "red"   ## #1f77b4
            # elif kpi_value > 0:
            #     value_color = "green"
            # else:
            #     value_color = "blue"
            ...
            with col:
                ...
                # Affichage du KPI avec label et valeur formatée
...
class BRSMAExpenses:
    def __init__(self, data, kpis):
        ...
        ### Regrouper et remplacer les mots similaires dans la colonne 'organisme'###
    ...
    def display_expenses_by_category(self):
        # st.write("**BRSMA** : Breakdown of expenditure by category")
    ...
    def display_average_transaction_transaction(self):
        # st.write("**BRSMA** : Average transaction per transaction")
    ...
    def display_top_suppliers(self):
        # st.write("**BRSMA** : Top 5 Creditors")
    ...
    def display_expenses_by_transaction_type(self):
        # st.write("**BRSMA** : Expenses by Transaction Type")
    ...
    def display_expenses_over_time(self):
        # st.write("**BRSMA** : Expenses over time")
    ...
    def display_expenses_by_day(self):
        # st.write("**BRSMA** : Expenses by Day of the Week")
    ...
    def display_organism_metrics(self):
        ...
        if search_query_brsma:
            filtered_data = self.brsma_metrique_org()[self.brsma_metrique_org().apply(lambda row: row.astype(str).str.contains(search_query_brsma, case=False).any(), axis=1)]
            occurrence = self.brsma_metrique_org().astype(str).apply(lambda x: x.str.contains(search_query_brsma, case=False)).sum().sum()
            st.write(f"*Search results [rows : {filtered_data.shape[0]}, occurrences : {occurrence}]*")
            
            # Calcul des KPI à partir des données filtrées
            total_organismes = filtered_data['organisme'].nunique()
            total_transactions = filtered_data['nombre'].sum()
            montant_total = filtered_data['montant_total'].sum()
            montant_moyenne = filtered_data['montant_moyenne'].mean()
            ...
            # Affichage des données filtrées
            montant_columns = filtered_data.filter(regex='^montant').columns
            for col in montant_columns:
                filtered_data[col] = filtered_data[col].apply(self.format_currency)
