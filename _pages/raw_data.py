...
class LCLDetails:
    ...
    def display_summary(self, search_query):
        with st.expander("🗄️ LCL Events : [raw data]"):
            ...
            if 'date_op_min_lcl' not in st.session_state:
                st.session_state['date_op_min_lcl'] = self.data["dateOp"].min().date()
            if 'date_op_max_lcl' not in st.session_state:
                st.session_state['date_op_max_lcl'] = self.data["dateOp"].max().date()
            ...
            with col2:
                ...
                date_op_min_lcl = st.date_input("dateOp min. LCL :", min_value=self.data["dateOp"].min().date(), max_value=self.data["dateOp"].max().date(), value=st.session_state['date_op_min_lcl'], key='date_op_min_lcl')
                date_op_max_lcl = st.date_input("dateOp max. LCL :", min_value=self.data["dateOp"].min().date(), max_value=self.data["dateOp"].max().date(), value=st.session_state['date_op_max_lcl'], key='date_op_max_lcl')

                # Mettre à jour les valeurs de session_state après sélection de la plage de dates
                if date_op_min_lcl != st.session_state['date_op_min_lcl']:
                    st.session_state['date_op_min_lcl'] = date_op_min_lcl
                if date_op_max_lcl != st.session_state['date_op_max_lcl']:
                    st.session_state['date_op_max_lcl'] = date_op_max_lcl

                # Filtrage des données par plage de dates sélectionnée
                filtered_data = self.data[
                    (self.data['dateOp'] >= pd.Timestamp(st.session_state['date_op_min_lcl'])) &
                    (self.data['dateOp'] <= pd.Timestamp(st.session_state['date_op_max_lcl']))
                ]
...
class BRSMAExpenses:
    ...
    def display_summary(self, search_query):
        with st.expander("🗄️ BRSMA Events : [raw data]"):
            ...
            with col2:
                # Calendrier interactif pour sélectionner la plage de dates
                date_min_brsma = st.date_input("dateOp min.  BRSMA", min_value=self.data["dateOp"].min().date(), max_value=self.data["dateOp"].max().date(), value=st.session_state['date_min_brsma'], key='date_min_brsma')
                date_max_brsma = st.date_input("dateOp max. BRSMA", min_value=self.data["dateOp"].min().date(), max_value=self.data["dateOp"].max().date(), value=st.session_state['date_max_brsma'], key='date_max_brsma')
